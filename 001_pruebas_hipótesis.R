...
val_crit=qnorm(p=alpha, mean= mu, sd=sdm, lower.tail = F) ## valor critico para alpha 0.05
...
val_p=pnorm(xbarra, mean=mu, sd=sdm, lower.tail = F) ### probabilidad de que con ese estadístico Ho se cumpla (probabilidad de equivocarse si rechaza Ho dado ese esttadistico) 
